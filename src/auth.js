 ...
 */
const LOGIN_URL = 'authenticate.json'
 ...
 */
const REFRESH_TOKEN_URL = '/authenticate.json'
...
export default {
    ...
    login (creds, redirect) {
        let base64Credentials = btoa(creds.username+":"+creds.password);
        AUTH_BASIC_HEADERS.headers.Authorization = 'Basic ' + base64Credentials
        return Vue.http.get(LOGIN_URL, AUTH_BASIC_HEADERS)
            .then((response) => {
    ...
    _storeToken (response) {
        ...
        auth.accessToken = response.body.data.token
        auth.refreshToken = response.body.data.refresh_token
        // TODO: get user's name from response from Oauth server.
        user.name = jwtDecode(auth.accessToken).username
